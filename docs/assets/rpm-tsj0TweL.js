    ...
    if (typeof n != 'string' && !Array.isArray(n)) {
      for (const t in n)
        if (t !== 'default' && !(t in a)) {
          ...
          i && Object.defineProperty(a, t, i.get ? i : { enumerable: !0, get: () => n[t] });
        }
    }
  }
...
(function (a, f) {
  (function (e) {
    ...
    e.defineMode('rpm-changes', function () {
      ...
      return {
        token: function (c) {
          return c.sol() && (c.match(n) || c.match(t))
            ? 'tag'
            : c.match(i)
              ? 'string'
              : (c.next(), null);
      ...
      e.defineMode('rpm-spec', function () {
        ...
        return {
          ...
          token: function (r, o) {
            var h = r.peek();
            if (h == '#') return r.skipToEnd(), 'comment';
            if (r.sol()) {
              if (r.match(t)) return 'header';
              if (r.match(i)) return 'atom';
            }
            if (r.match(/^\$\w+/) || r.match(/^\$\{\w+\}/)) return 'def';
            if (r.match(l)) return 'keyword';
            if (r.match(c)) return (o.controlFlow = !0), 'keyword';
            if (o.controlFlow) {
              if (r.match(u)) return 'operator';
              if (r.match(/^(\d+)/)) return 'number';
              r.eol() && (o.controlFlow = !1);
            }
            if (r.match(n)) return r.eol() && (o.controlFlow = !1), 'number';
            if (r.match(/^%[\w]+/)) return r.match('(') && (o.macroParameters = !0), 'keyword';
            if (o.macroParameters) {
              if (r.match(/^\d+/)) return 'number';
              if (r.match(')')) return (o.macroParameters = !1), 'keyword';
            }
            return r.match(/^%\{\??[\w \-\:\!]+\}/)
              ? (r.eol() && (o.controlFlow = !1), 'def')
              : (r.next(), null);
          },
        };
      }),
...
var p = g.exports;
const w = d(p),
  b = m({ __proto__: null, default: w }, [p]);
export { b as r };
